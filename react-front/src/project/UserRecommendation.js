...
class UserRecommendation extends Component {
  ...
  { 
    ...
    users.sort(function(a,b){
      return (b.projects.length + b.completed_projects.length) - (a.projects.length + a.completed_projects.length);
    });
    ...
    users.forEach((user) => 
    {
      ...
      let total_projects = (user.projects.length+user.completed_projects.length);
      let final_value = (total_projects/max_proj*user.rating/10) + (score.exact/proj_skills.length) + (-0.04*user.projects.length) + (0.01*user.completion_percentage_of_all_projects);
    ...
    final_out.sort(function(a, b){return b.final_value - a.final_value});
